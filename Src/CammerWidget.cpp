...
static void deviceOnLine(IMV_HANDLE devHandle)
{
    ...
    ret = IMV_SubscribeConnectArg(devHandle, onDeviceLinkNotify, NULL);
    if (IMV_OK != ret)
    {
...
static void onDeviceLinkNotify(const IMV_SConnectArg* pConnectArg, void* pUser)
{
    ...
    if (NULL == pCammerWidget->m_devHandle)
    {
    ...
    ret = IMV_GetDeviceInfo(pCammerWidget->m_devHandle, &devInfo);
    if (IMV_OK != ret)
    {
    ...
    {
        qDebug()<<"------cameraKey[%s] : OffLine------\n"<<devInfo.cameraKey;
//        deviceOffLine(devHandle);
        pCammerWidget->CameraStop();
    }
    // 上线通知
    // onLine notify
    else if (onLine == pConnectArg->event)
    {
        qDebug()<<"------cameraKey[%s] : OnLine------\n"<< devInfo.cameraKey;
//        deviceOnLine(devHandle);
        // 关闭相机
        // Close camera
        IMV_Close(pCammerWidget->m_devHandle);
        ...
        do
        {
            ...
            ret = IMV_Open(pCammerWidget->m_devHandle);
            if (IMV_OK != ret)
            {
                printf("Retry open camera failed! ErrorCode[%d]\n", ret);
            }
...
static void FrameCallback(IMV_Frame* pFrame, void* pUser)
{
	...
	if (frameInfo.m_pImageBuf != NULL)
	{
        ...
        qDebug()<<"emory application failed, return directly";
    }
...
bool CammerWidget::CameraStart()
{
    ...
    ret = IMV_SubscribeConnectArg(m_devHandle, onDeviceLinkNotify, this);
    if (IMV_OK != ret)
    {
        printf("SubscribeConnectArg failed! ErrorCode[%d]\n", ret);
        return false;
